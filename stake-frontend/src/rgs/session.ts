...
export function parseLaunch(
  ...
  if (!sessionID || !rgsUrl) {
    ...
    isLocal = !rgsUrl || /^(localhost|127\.0\.0\.1)(:|\/|$)/.test(rgsUrl);
    ...
    sessionID = sessionID || (isDev && !isReplayMode ? devStableSessionID() ||
  ...
  return { sessionID, rgsUrl, rgsBase, lang, device, currencyHint, isLocal, isReplayMode, replayEvent, replayAmount, replayMode, replayGame, replayVersion, social };
}
